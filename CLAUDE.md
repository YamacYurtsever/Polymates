...
### Database
- [x] Add `share_token uuid not null default gen_random_uuid()` to `bets` table
- [x] RPC `get_bet_by_share_token(p_token uuid)` — security definer, returns bet id, title, group id, group name, closes_at, status; no auth required (bypasses RLS so anyone with the link can preview)
- [x] Reuse existing `group_members` insert for the group-join step on the share page

### Frontend — Share Page (`/share/[token]`)
- [x] Fetch bet + group info via `get_bet_by_share_token` RPC
- [x] **Not signed in**: show bet title, group name, and "Sign up / Log in to participate" CTA — auth pages receive a `?next=/share/[token]` param and redirect back after login
- [x] **Signed in, not in group**: show bet title + group name + "Join [group] & View Bet" button — joins group then redirects to `/bets/[id]`
- [x] **Signed in, already in group**: immediately redirect to `/bets/[id]`
- [x] Handle `?next=` redirect param in Login and SignUp pages (already supported)
...
Goal: resolve bets automatically even when no user has the bet page open. Currently resolution only fires when a client detects the deadline has passed. The cron job closes this gap.

### Design

- `pg_cron` (built into Supabase) schedules a job every minute
- The job calls a Postgres function `resolve_overdue_bets()` which loops over all `open` bets past their `closes_at` and fires the existing `resolve-bet` edge function for each via `pg_net` (async HTTP)
- No status change happens in the cron function itself — `resolve-bet` already handles the full lifecycle (verdict write + `resolve_bet()` payout SQL + status update)
- Safe to run concurrently with live clients: the edge function checks for an existing verdict and returns early; the `verdicts` table has a `unique` constraint on `bet_id` as a hard backstop
- Service role key stored in `vault.secrets`, never hardcoded in SQL

### Prerequisites (Supabase dashboard)
- [ ] Enable `pg_net` extension (Database → Extensions)
- [ ] Enable `pg_cron` extension (Database → Extensions)
- [ ] Add service role key to vault: `select vault.create_secret('SERVICE_ROLE_KEY', '<key>', 'service_role_key');`
- [ ] Note the project URL — needed in the migration

### Database — `backend/migrations/m8_cron.sql`
- [ ] Create `resolve_overdue_bets()` — security definer function that:
  - Reads project URL and service role key from `vault.decrypted_secrets`
  - Selects all `bets` where `closes_at < now()` and `status = 'open'`
  - For each, calls `net.http_post()` to `/functions/v1/resolve-bet` with `{"bet_id": "..."}` body and `Authorization: Bearer <key>` header
- [ ] Schedule with `cron.schedule('resolve-overdue-bets', '* * * * *', 'select public.resolve_overdue_bets()')`
- [ ] Grant `execute` on `resolve_overdue_bets()` to `postgres`
